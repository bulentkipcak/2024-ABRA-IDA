...
cap = cv2.VideoCapture("http://localhost:8080/stream?topic=/webcam/image_raw")
...
def update_json_angle(angle):
    data = {'error_yaw': angle}
    with open('error_angle.json', 'w') as f:
...
def update_json_vehicle_state(vehicle_state):
    """ Update vehicle_state.json file with the given angle """
